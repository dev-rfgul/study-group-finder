...
const Home = () => {
    ...
    const handleJoinGroup = async (groupId) => {
        ...
        try {
            setIsLoading(true)
            const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/joinGroup`, {
                userId: user?.id,
                groupId,
            });
    ...
    const handleGroupClick = async (group) => {
        ...
        try {
            // Fetch messages for the selected group
            const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/getGroupByID/${groupID}`);
            ...
            if (response.status === 200) {
                ...
                const messagesWithUsernames = await Promise.all(response.data.messages.map(async (msg) => {
                    try {
                        // Fetch the user by their ID
                        const userResponse = await axios.get(`${import.meta.env.VITE_BASE_URL}/getUserByID/${msg.userID}`);
    ...
    const handleSendMessage = async () => {
        ...
        const url = `${import.meta.env.VITE_BASE_URL}/sendMessage`; // Backend endpoint
    ...
    const handleRemoveGroup = async (e) => {
        ...
        try {
            ...
            const url = `${import.meta.env.VITE_BASE_URL}/removeGroup/${activeGroup._id}/${userID}`;
            setIsLoading(true)
            console.log(url);
            const response = await axios.post(url);
